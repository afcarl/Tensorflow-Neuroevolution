...
def main():
    ...
    process to evolve a genome to solve the XOR environment by evolving it with the TEWT algorithm.
    """
    ...
    config = ne.load_config('./xor_tewt_example.cfg')

    environment = ne.environments.XORWeightTrainingEnvironment(config)
    ne_algorithm = ne.algorithms.TEWT(config, dtype=tf.float32, run_eagerly=False)
    ...
    best_genome = engine.train(reporting_agents=None)
