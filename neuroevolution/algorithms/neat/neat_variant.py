...
class NEATVariant(BaseNeuroevolutionAlgorithm):
    """
    Variant implementation of Kenneth O'Stanleys and Risto Miikkulainen's NEAT algorithm.
    The algorithm differs in extended and at times different functionality than standard NEAT's. All differences are
    listed and documented in TFNE's documentation section for NEAT-Variant.
    """

    def __init__(self, config, dtype=tf.float32, run_eagerly=False):
        # Declare, read in and log config parameters for the NEAT-Variant algorithm
        ...
        # Use DirectEncoding with the supplied parameters. Set trainable to False as NEAT-Variant is training/evolving
        # the weights itself
        ...
        # assign all newly initialized genomes to species 1.
        ...
        # Initialize topology counter specifics, keeping track of unique topologies and assigning each genome_id a
        # topology id
        self.topology_id_counter = 0
        self.topology_id_history = dict()
        self.genome_topology_id = dict()
    ...
    def _read_config_parameters(self, config):
        ...
        """
        section_name_algorithm = 'NEAT-Variant' if config.has_section('NEAT-Variant') else 'NE_ALGORITHM'
    ...
    def _log_class_parameters(self):
        logging.debug("NEAT-Variant algorithm config: reproducing_fraction = {}".format(self.reproducing_fraction))
        logging.debug("NEAT-Variant algorithm config: crossover_prob = {}".format(self.crossover_prob))
        logging.debug("NEAT-Variant algorithm config: mutation_weights_prob = {}".format(self.mutation_weights_prob))
        logging.debug("NEAT-Variant algorithm config: mutation_add_conn_prob = {}".format(self.mutation_add_conn_prob))
        logging.debug("NEAT-Variant algorithm config: mutation_add_node_prob = {}".format(self.mutation_add_node_prob))
        logging.debug("NEAT-Variant algorithm config: mutation_weights_fraction = {}"
                      .format(self.mutation_weights_fraction))
        logging.debug("NEAT-Variant algorithm config: mutation_weights_mean = {}".format(self.mutation_weights_mean))
        logging.debug("NEAT-Variant algorithm config: mutation_weights_stddev = {}"
                      .format(self.mutation_weights_stddev))
        logging.debug("NEAT-Variant algorithm config: distance_excess_c1 = {}".format(self.distance_excess_c1))
        logging.debug("NEAT-Variant algorithm config: distance_disjoint_c2 = {}".format(self.distance_disjoint_c2))
        logging.debug("NEAT-Variant algorithm config: distance_weight_c3 = {}".format(self.distance_weight_c3))
        logging.debug("NEAT-Variant algorithm config: species_elitism = {}".format(self.species_elitism))
        logging.debug("NEAT-Variant algorithm config: species_max_stagnation = {}".format(self.species_max_stagnation))
        logging.debug("NEAT-Variant algorithm config: species_clustering = {}".format(self.species_clustering))
        logging.debug("NEAT-Variant algorithm config: activation_hidden = {}".format(self.activation_hidden))
        logging.debug("NEAT-Variant algorithm config: activation_output = {}".format(self.activation_output))

    def initialize_population(self, population, initial_pop_size, input_shape, num_output):
        """
        Initialize the population with DirectEncoding genomes of minimal, fully-connected topologies (not hidden nodes,
        all inputs are connected to all outputs). The initial connection weights are randomized in the sense of them
        being mutated once (which means the addition of a value from a random normal distribution with cfg specified
        mean and stddev), while the node biases are all initialized to 0.
        ...
        if len(input_shape) == 1:
            ...
            for _ in range(initial_pop_size):
                ...
                # Keep track of topologies and assign each genome a topology id
                topology = frozenset(genotype.keys())
                if topology in self.topology_id_history:
                    self.genome_topology_id[new_genome_id] = self.topology_id_history[topology]
                else:
                    self.topology_id_counter += 1
                    self.topology_id_history[topology] = self.topology_id_counter
                    self.genome_topology_id[new_genome_id] = self.topology_id_counter
    ...
    def evolve_population(self, population, pop_size_fixed):
        ...
        # is 1. This is necessary as the NEAT formula for allotted offspring allows to increase the population, though
        # the NEAT-Variant algorithm is specified on a fixed population size. To counteract this will the species with
        # the most/least allotted offsprign add/subtract one potential genome from its allotted size.
        allotted_offspring = dict()
        for species_id, species_genome_ids in self.species_assignment.items():
            allotted_offspring[species_id] = round(species_adj_fitness_average[species_id] * pop_size /
                                                   total_adj_fitness_avg) - 1
        ...
        for species_id, species_genome_ids in self.species_assignment.items():
            ...
            for _ in range(allotted_offspring[species_id]):
                ...
                # Keep track of topologies and assign each genome a topology id
                topology = frozenset(new_genome.get_genotype().keys())
                if topology in self.topology_id_history:
                    self.genome_topology_id[new_genome_id] = self.topology_id_history[topology]
                else:
                    self.topology_id_counter += 1
            ...
            # Delete all parent genomes of species as generations completely replace each other, though keep best genome
            # as genome_elitism currently fixed to 1.
            for species_genome_id in species_genome_ids[1:]:
                ...
                del self.genome_topology_id[species_genome_id]
    ...
    def _create_added_node_genome(self, parent_genome) -> (int, DirectEncodingGenome):
        ...
        # Extract all required information from chosen gene to build a new node in between and then disable it. If
        # between the conn_in and conn_out has already been another node added in another mutation, use the same node
    ...
    def evaluate_population(self, population, genome_eval_function):
        ...
        saving its fitness. The population is the clustered and fitness sharing is applied.
        :param population: Population object of the TFNE framework, serving as an abstraction for the genome collection
    ...
    def summarize_population(self, population):
        ...
        for species_id, species_genome_ids in self.species_assignment.items():
            ...
            for genome_id in species_genome_ids:
                logging.debug(str(population.get_genome(genome_id)) + " || Topology ID: {:>4}"
                              .format(self.genome_topology_id[genome_id]))
