...
class TEWT(BaseNeuroevolutionAlgorithm):
    """
    TEWT - 'Topology Evolving Weight Training' NE algorithm
    """
    ...
    def __init__(self, config, dtype=tf.float32, run_eagerly=False):
        ...
        assert self.crossover_prob + self.mutation_add_conn_prob + self.mutation_add_node_prob == 1.0

        # Use DirectEncoding with the supplied parameters
        self.encoding = ne.encodings.DirectEncoding(trainable=True, dtype=dtype, run_eagerly=run_eagerly)
    ...
    def _read_config_parameters(self, config):
        ...
        """
        section_name = 'TEWT' if config.has_section('TEWT') else 'NE_ALGORITHM'
    ...
    def _log_class_parameters(self):
        logging.debug("TEWT algorithm config: reproducing_fraction = {}".format(self.reproducing_fraction))
        logging.debug("TEWT algorithm config: crossover_prob = {}".format(self.crossover_prob))
        logging.debug("TEWT algorithm config: mutation_add_conn_prob = {}".format(self.mutation_add_conn_prob))
        logging.debug("TEWT algorithm config: mutation_add_node_prob = {}".format(self.mutation_add_node_prob))
        logging.debug("TEWT algorithm config: distance_excess_c1 = {}".format(self.distance_excess_c1))
        logging.debug("TEWT algorithm config: distance_disjoint_c2 = {}".format(self.distance_disjoint_c2))
        logging.debug("TEWT algorithm config: distance_weight_c3 = {}".format(self.distance_weight_c3))
        logging.debug("TEWT algorithm config: species_elitism = {}".format(self.species_elitism))
        logging.debug("TEWT algorithm config: species_max_stagnation = {}".format(self.species_max_stagnation))
        logging.debug("TEWT algorithm config: species_clustering = {}".format(self.species_clustering))
        logging.debug("TEWT algorithm config: activation_hidden = {}".format(self.activation_hidden))
        logging.debug("TEWT algorithm config: activation_output = {}".format(self.activation_output))
        ...
        if len(input_shape) == 1:
            ...
            for _ in range(initial_pop_size):
                ...
                for node in range(num_input + 1, num_input + num_output + 1):
                    ...
                    gene_id, gene_node = self.encoding.create_gene_node(node, 0.5, self.activation_output)
    ...
    def evolve_population(self, population, pop_size_fixed):
        ...
        mutation_add_conn_randval = self.crossover_prob + self.mutation_add_conn_prob
        ...
        for species_id, species_genome_ids in self.species_assignment.items():
            ...
            # champion as genome elitism set to 1
    ...
    def _create_added_node_genome(self, parent_genome) -> (int, DirectEncodingGenome):
        ...
        gene_node_id, gene_node = self.encoding.create_gene_node(node, 0.5, self.activation_hidden)
